...
<html lang="en">
    ...
    <body>
        ...
        <script defer async>
            window.putLog = (message) => {
                let logContainer = document.querySelector('[data-id="log"]');

                if (!logContainer) {
                    return;
                }

                let previousContent = logContainer.innerText;
                logContainer.innerText = message + (previousContent ? `\n${previousContent}` : '');
            }

            document.addEventListener('DOMContentLoaded', (event) => {
                window.Echo.channel('room.my-room-0123').listen('MessageSent', (e) => {
                    console.log(JSON.stringify(e, null, 4) + `\n`);
                    putLog(JSON.stringify(e, null, 4) + `\n`);
                });
            });
            ...
            document.addEventListener('alpine:init', () => {
                ...
                Alpine.data('chatPageData', () => ({
                    myId: 'tiago',
                    myTimeZone: 'America/Sao_Paulo',
                    messages: [
                        // '2024-02-20T04:47:24+00:00'
                        {
                            author: {
                                id: 'pedro',
                                name: 'Pedro',
