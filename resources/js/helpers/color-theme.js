...
export const setTheme = (theme = null) => {
    ...
    if (theme != 'dark' && document.querySelector('html')) {
        document.querySelector('html').classList.remove('dark');
        cookieManager.eraseCookie('color-theme');
    }

    cookieManager.setCookie('color-theme', getCurrentTheme());
...
export const listenSchemeChange = () => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('button[data-switch-theme]')
            .forEach(element => element.addEventListener('click', (event) => {
                event.preventDefault();
                event.stopPropagation();

                if (!window.colorTheme || !window.colorTheme?.switchTheme) {
                    return;
                }

                window.colorTheme.switchTheme();
            }))
    });
}
